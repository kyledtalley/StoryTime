...
const VOICE_ID = "yxzqv7jXBIZY16OevUlW" // Replace with your preferred voice ID
...
const fetchElevenLabsTTS = async (text) => {
	...
	try {
		const response = await axios.post(
			`${API_BASE_URL}/text-to-speech/${VOICE_ID}?enable_logging=true&optimize_streaming_latency=1&output_format=mp3_44100_192`,
			options.data,
			{
...
export const readStory = async (story) => {
	try {
		const audioPath = await fetchElevenLabsTTS(story)
